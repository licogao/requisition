...
export default function App() {
  ...
  const handleBatchAction = (actionType) => {
    ...
    if (actionType === 'advance') {
        ...
        setModal({
            ...
            onConfirm: async ({ note }) => { await executeBatchUpdate(targets, 'advance', note); }
        });
        ...
        setModal({
            isOpen: true, type: 'action', alertType: 'danger', title: `批量退回 (${validTargets.length} 筆)`, message: `將退回 ${validTargets.length} 筆單據。\n(已自動略過 ${targets.length - validTargets.length} 筆初始階段單據)\n\n請輸入退回原因`, showNoteInput: true, noteRequired: true,
            onConfirm: async ({ note }) => { await executeBatchUpdate(validTargets, 'revert', note); }
        });
        ...
        setModal({
            ...
            onConfirm: async ({ pickupName }) => { await executeBatchUpdate(validTargets, 'receiver', null, pickupName); }
        });
    }
  };
  ...
  const executeBatchUpdate = async (targets, type, note, pickupName) => {
    ...
    if (successCount > 0) {
        await batch.commit();
        const logType = (type === 'advance' || type === 'revert' || type === 'receiver') ? LOG_TYPES.STATUS_CHANGE : LOG_TYPES.UPDATE;
  ...
  const handleCloudSearch = async () => {
    if (!searchTerm.trim()) { setModal({ isOpen: true, type: 'alert', title: '請輸入關鍵字', message: '請輸入流水號（例如 11201-01）或廠商名稱以進行雲端搜尋。' }); return; }
    ...
    } catch (err) { console.error("Cloud search error:", err); setModal({ isOpen: true, type: 'alert', alertType: 'danger', title: '搜尋失敗', message: '連接資料庫時發生錯誤。' }); } finally { setIsSearchingCloud(false); }
  };
