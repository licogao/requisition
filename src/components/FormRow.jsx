...
import { 
  Clock, PlayCircle, CheckCircle, AlertCircle, ArrowRight, RotateCcw, XCircle, Edit2, FileText, User, Building, Briefcase, Truck, CheckSquare, Copy 
} from 'lucide-react';
    ...
    <>
      <tr className={`border-b border-slate-100 hover:bg-blue-50/50 transition-colors ${isExpanded ? 'bg-blue-50/30' : ''} ${form.isUrgent ? 'bg-red-50 hover:bg-red-100' : 'hover:bg-blue-50/50'} ${statusConfig.phase === 4 ? 'opacity-60 grayscale-[50%]' : ''}`}>
        <td className="p-4 text-center w-12">
          <input type="checkbox" checked={selected} onChange={(e) => onSelect(form.id, e.target.checked)} className="w-5 h-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500 cursor-pointer" />
        </td>
        <td className="p-4 align-top cursor-pointer" onClick={() => setExpandedId(isExpanded ? null : form.id)}>
          ...
          {form.isUrgent && <span className="inline-block mt-2 px-2 py-0.5 bg-red-100 text-red-600 text-xs font-bold rounded border border-red-200 animate-pulse">速件</span>}
        </td>
        <td className="p-4 align-top cursor-pointer" onClick={() => setExpandedId(isExpanded ? null : form.id)}>
          ...
          <div className="flex items-center gap-2 text-sm text-slate-500 flex-wrap">
            <span className="bg-slate-100 px-2 py-0.5 rounded text-xs">預算: ${parseFloat(form.totalPrice || 0).toLocaleString()}</span>
        ...
        <td className="p-4 align-top">
          <div className="flex flex-col gap-2">
            {statusConfig.phase < 3 && (
              <button onClick={(e) => { e.stopPropagation(); onAction('advance', form); }} className="flex items-center justify-center gap-1 px-3 py-1.5 bg-emerald-600 text-white rounded hover:bg-emerald-700 text-xs font-bold shadow-sm transition-all whitespace-nowrap">
                {statusConfig.nextAction} <ArrowRight size={14} />
              </button>
            )}
            {canRevert && (
               <button onClick={(e) => { e.stopPropagation(); onAction('revert', form); }} className="flex items-center justify-center gap-1 px-3 py-1.5 bg-red-50 border border-red-200 text-red-600 rounded hover:bg-red-100 text-xs font-bold transition-all whitespace-nowrap">
        ...
        <td className="p-4 text-center align-top">
           <button onClick={(e) => { e.stopPropagation(); onAction('delete', form); }} className="p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded transition-colors" title="刪除"><XCircle size={18} /></button>
        </td>
      </tr>
      {isExpanded && (
          ...
          <td colSpan="6" className="p-0">
            <div className="flex flex-col md:flex-row">
               <div className="flex-1 p-6 relative">
                  <div className="absolute top-4 right-4 flex gap-2 z-10">
                      {statusConfig.phase !== 4 && (
                          <>
                             <button onClick={() => onAction('void_and_replace', form)} className="flex items-center gap-1.5 px-3 py-1.5 bg-white text-slate-600 border border-slate-200 rounded-full hover:bg-slate-50 hover:shadow-md transition-all text-sm font-bold shadow-sm">
                                <Copy size={16} /> 換單作廢
                             </button>
                             <button onClick={() => onAction('edit', form)} className="flex items-center gap-1.5 px-3 py-1.5 bg-white text-blue-600 border border-blue-200 rounded-full hover:bg-blue-50 hover:shadow-md transition-all text-sm font-bold shadow-sm">
                                <Edit2 size={16} /> 修改
                             </button>
                          </>
                      )}
                      {statusConfig.phase === 4 && (
                           <button onClick={() => onAction('undo_void', form)} className="flex items-center gap-1.5 px-3 py-1.5 bg-white text-emerald-600 border border-emerald-200 rounded-full hover:bg-emerald-50 hover:shadow-md transition-all text-sm font-bold shadow-sm">
                              <RotateCcw size={16} /> 解除作廢
                           </button>
                      )}
                  </div>
                  <h4 className="font-bold text-slate-700 text-xl mb-6 flex items-center gap-2 border-b border-slate-200 pb-3 pr-48">
                  ...
                  <div className="bg-white rounded border border-slate-200 overflow-hidden mb-4 shadow-sm">
                    <table className="w-full text-base">
                      <thead className="bg-slate-50 text-slate-600 font-bold border-b">
                        <tr><th className="p-3 text-left">品項名稱</th><th className="p-3 text-right">單價</th><th className="p-3 text-center">數量</th><th className="p-3 text-right">小計</th></tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100">
                        {(form.items || []).map((item, idx) => (
                          <tr key={idx}>
                            <td className={`p-3 text-slate-800 font-medium ${statusConfig.phase === 4 ? 'line-through' : ''}`}>{item.subject}</td>
                            <td className="p-3 text-right font-mono">${parseFloat(item.unitPrice || 0).toLocaleString()}</td>
                            <td className="p-3 text-center">{item.quantity} {item.measureUnit}</td>
                            <td className="p-3 text-right font-bold">${parseFloat(item.subtotal || 0).toLocaleString()}</td>
                          </tr>
                        ))}
                        <tr className="bg-slate-50 font-bold">
                            <td colSpan="3" className="p-3 text-right">總金額</td>
                            <td className={`p-3 text-right text-xl ${statusConfig.phase === 4 ? 'text-slate-400 line-through' : 'text-blue-700'}`}>${parseFloat(form.totalPrice || 0).toLocaleString()}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
               ...
               <div className={`w-full md:w-1/3 border-l border-slate-200 p-6 ${form.isUrgent ? 'bg-red-50' : 'bg-slate-50'}`}>
                  <h4 className="font-bold text-slate-600 border-b pb-3 mb-5 flex items-center gap-2 text-lg"><Clock size={20} /> 操作歷程</h4>
