        ...
        <td className="p-4 align-top cursor-pointer" onClick={() => setExpandedId(isExpanded ? null : form.id)}>
          ...
          <div className="flex items-center gap-2 text-sm text-slate-500 flex-wrap">
            <span className="bg-slate-100 px-2 py-0.5 rounded text-xs">
               預算: <span className={statusConfig.phase === 4 ? 'line-through text-slate-400' : ''}>${parseFloat(form.totalPrice || 0).toLocaleString()}</span>
            </span>
                  ...
                  <div className="space-y-4 mb-6">
                    ...
                    {form.globalRemark && (
                        <div className="flex items-start gap-3 mt-1">
                            <span className="w-24 text-sm font-bold text-slate-500 text-right uppercase tracking-wider mt-1.5">案件備註</span>
                            <div className="bg-yellow-50 text-yellow-700 px-4 py-1.5 rounded-2xl border border-yellow-200 text-base whitespace-pre-wrap w-fit flex items-start gap-1.5 shadow-sm">
                                <FileText size={18} className="mt-0.5 shrink-0" />
                                <span className="leading-relaxed font-medium">{form.globalRemark}</span>
                            </div>
                        </div>
                    )}
                  </div>
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {(form.items || []).map((item, idx) => (
                          <tr key={idx}>
                            ...
                            <td className="p-3 text-right font-mono text-slate-600">${parseFloat(item.unitPrice).toLocaleString()}</td>
                            <td className="p-3 text-center text-slate-700">{item.quantity} {item.measureUnit}</td>
                            <td className="p-3 text-right font-bold text-slate-900">${parseFloat(item.subtotal).toLocaleString()}</td>
                          </tr>
                        ))}
